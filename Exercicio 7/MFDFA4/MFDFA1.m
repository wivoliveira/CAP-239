...
if Fig==1, 
   ...
   %---------------------------
    ...
    % Create textbox
    [Dq_pico, i_pico] = max(Dq);
    hq_pico = hq(i_pico);
    
    annotation(figure1,'textbox',[0.5855 0.2269 0.2413 0.05518],...
        'String',{strcat('Psi = (hq_m_a_x - hq_m_i_n)/hq_p_i_c_o', sprintf('\nPsi = ('), num2str(max(hq)), ' - ', num2str(min(hq)), ')/', num2str(hq_pico), sprintf('\nPsi = '),num2str((max(hq)-min(hq))/hq_pico))},...
        'HorizontalAlignment','center',...
        'FontSize',14,...
        'FitBoxToText','off',...
        'LineStyle','none');
