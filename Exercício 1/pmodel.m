...
if ( ~isempty(slope) )
    ...
    x = fourierCoeff .* exp(1i*phase); % Calculate the complex Fourier coefficients with the specified spectral slope, and the phases of the p-model.
...
% Para salvar o csv, use:
%csvwrite('s8.csv', transpose(s8));
%csvwrite('s9.csv', transpose(s9));
%csvwrite('s10.csv', transpose(s10));
